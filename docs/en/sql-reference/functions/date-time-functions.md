...
| %f       | fractional second from the fractional part of DateTime64, see below (*) | 1234560   |
...
(*) The behavior of `%f` is to print `000000` (six zeros) if the formatted value is a Date, Date32 or DateTime (all of which have no fractional seconds) or a DateTime64 with a precision of 0. Earlier versions of ClickHouse printed `0` in this case. The previous behavior can be restored by setting `formatdatetime_f_prints_single_zero = 1`.
